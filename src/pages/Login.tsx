    ...
    <div style={{
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: Spacing.md }}>
        <div style={{
          ...
          <input
            ...
            style={{ flex: 1, border: 'none', outline: 'none', fontSize: Font.body, backgroundColor: 'transparent', color: Colors.ink }}
          />
        </div>
        ...
        {!isSignUp && (
          <button type="button" onClick={handleForgotPassword}
            style={{ alignSelf: 'flex-end', color: Colors.primary, fontSize: Font.sm, fontWeight: Weight.semibold, background: 'none', border: 'none', cursor: 'pointer' }}>
            {t.auth.forgotPassword}
          </button>
        )}

        {error && (
          <div style={{ padding: Spacing.md, borderRadius: Radius.sm, backgroundColor: Colors.errorPale, color: Colors.error, fontSize: Font.sm, fontWeight: Weight.semibold }}>
            {error}
          </div>
        )}
        {resetSent && (
          <div style={{ padding: Spacing.md, borderRadius: Radius.sm, backgroundColor: Colors.successPale, color: Colors.success, fontSize: Font.sm, fontWeight: Weight.semibold }}>
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? <div className="spinner" /> : (
            <>{isSignUp ? t.auth.signUp : t.auth.signIn}<ArrowRight size={20} /></>
          )}
        </button>
      </form>
      ...
      <div style={{ textAlign: 'center', marginTop: Spacing.lg }}>
        ...
        <button onClick={() => { setIsSignUp(!isSignUp); setError(''); }}
          style={{ fontSize: Font.sm, fontWeight: Weight.bold, color: Colors.primary, background: 'none', border: 'none', cursor: 'pointer' }}>
