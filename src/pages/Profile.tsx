  ...
  return (
    <div style={{ backgroundColor: Colors.background, minHeight: '100vh' }}>
      ...
      <div className="fade-in" style={{ padding: 16 }}>
        ...
        {/* Badges preview */}
        <SectionHeader title={t.profile.myBadges} actionLabel={t.common.seeAll} onAction={() => navigate('/challenges')} />
        <Card className="card-interactive" onClick={() => navigate('/challenges')} style={{ display: 'flex', alignItems: 'center', gap: Spacing.md, marginBottom: 28, cursor: 'pointer' }}>
          <div style={{ display: 'flex', flex: 1 }}>
            {(earnedBadges.length > 0 ? BADGES.filter(b => earnedBadges.includes(b.id)) : BADGES).slice(0, 5).map((badge, i) => {
              ...
              return (
                <div key={badge.id} style={{
                  ...
                  border: `2px solid ${Colors.surface}`, marginLeft: i > 0 ? -12 : 0, position: 'relative', zIndex: 5 - i,
                }}>
                  <BadgeIcon size={18} color={earned ? Colors.primary : Colors.inkTertiary} style={{ opacity: earned ? 1 : 0.3 }} />
                </div>
              );
            })}
          </div>
          <span style={{ fontSize: Font.sm, color: Colors.inkSecondary, fontWeight: Weight.semibold, whiteSpace: 'nowrap' }}>
